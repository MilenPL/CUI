...
#polish language version | wersja polsko języczna
...
# Wymuszenie odpowiednich zmiennych dla Qt na Waylandzie / X11
...
try:
    ...
    print(f"[BŁĄD] Brak wymaganych bibliotek PyQt6: {e}")
    print("Zainstaluj je komendą: pip install PyQt6")
    sys.exit(1)

# Ścieżki do zasobów i konfiguracji
DATA_DIR = "CUI-DATA"
...
# Globalny motyw kolorystyczny (domyślny)
THEME_COLOR = QColor(0, 243, 255)
THEME_NAME = "Cyjan (Domyślny)"

COLOR_MAP = {
    "Cyjan (Domyślny)": QColor(0, 243, 255),
    "Zielony Neon": QColor(57, 255, 20),
    "Czerwony Cyber": QColor(255, 0, 85),
    "Fioletowy": QColor(170, 0, 255),
    "Pomarańczowy": QColor(255, 110, 0),
    "Żółty": QColor(255, 230, 0)
}
...
class IconCustomizeDialog(QDialog):
    def __init__(self, current_apps, parent=None):
        ...
        self.setWindowTitle("Dostosuj ikony pulpitu")
        self.resize(450, 520)
        ...
        layout.addWidget(QLabel("Zarządzaj ikonami na obręczy (max 12):"))
        ...
        self.btn_add = QPushButton("Dodaj własną...")
        self.btn_remove = QPushButton("Usuń zaznaczoną")
    ...
    def add_custom_icon(self):
        name, ok1 = QInputDialog.getText(self, "Nowa Ikona", "Wprowadź nazwę ikony:")
        if ok1 and name:
            cmd, ok2 = QInputDialog.getText(self, "Nowa Ikona", "Wprowadź polecenie/ścieżkę uruchamiania:")
            if ok2 and cmd:
...
class WallpaperSelectorDialog(QDialog):
    def __init__(self, current_wallpaper_path, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Wybór tapety")
        self.resize(450, 400)
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Dostępne tapety w katalogu grafic/wallpapers:"))
        ...
        btn_layout = QHBoxLayout()
        self.btn_add = QPushButton("Wgraj nową...")
    ...
    def upload_wallpaper(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Wybierz nową tapetę", "", "Obrazy (*.png *.jpg *.jpeg *.bmp *.webp)")
        if file_path:
            ...
            try:
                ...
                print(f"[BŁĄD] Błąd kopiowania: {e}")
...
class SettingsDialog(QDialog):
    def __init__(self, main_view, parent=None):
        ...
        self.setWindowTitle("Ustawienia Systemu")
        self.resize(550, 420)
        ...
        lay_wp.addWidget(QLabel("Zarządzanie tłem pulpitu:"))
        btn_open_wp = QPushButton("Otwórz Menedżer Tapet")
        btn_open_wp.clicked.connect(self.open_wp)
        lay_wp.addWidget(btn_open_wp)
        lay_wp.addStretch()
        self.tabs.addTab(tab_wallpaper, "Tapeta")

        # Tab 2: Kolor Przewodni
        tab_color = QWidget()
        lay_col = QVBoxLayout(tab_color)
        lay_col.addWidget(QLabel("Wybierz akcent kolorystyczny interfejsu:"))
        ...
        btn_apply_color = QPushButton("Zastosuj Kolor")
        btn_apply_color.clicked.connect(self.change_theme_color)
        lay_col.addWidget(btn_apply_color)
        lay_col.addStretch()
        self.tabs.addTab(tab_color, "Kolor")

        # Tab 3: Strefa Czasowa
        tab_tz = QWidget()
        lay_tz = QVBoxLayout(tab_tz)
        lay_tz.addWidget(QLabel("Ustaw strefę czasową:"))
        ...
        btn_apply_tz = QPushButton("Ustaw Strefę Czasową")
        btn_apply_tz.clicked.connect(self.change_timezone)
        lay_tz.addWidget(btn_apply_tz)
        lay_tz.addStretch()
        self.tabs.addTab(tab_tz, "Strefa Czasowa")

        # Tab 4: Użytkownicy Linux
        tab_users = QWidget()
        lay_usr = QVBoxLayout(tab_users)
        lay_usr.addWidget(QLabel("Zarządzanie Użytkownikami Systemu (Linux):"))
        self.user_list = QListWidget()
        lay_usr.addWidget(self.user_list)
        self.refresh_users()
        
        usr_btns = QHBoxLayout()
        btn_add_usr = QPushButton("Dodaj Użytkownika")
        btn_edit_usr = QPushButton("Edytuj Hasło")
        btn_del_usr = QPushButton("Usuń Użytkownika")
        ...
        self.tabs.addTab(tab_users, "Użytkownicy")

        # Tab 5: Pomoc
        tab_help = QWidget()
        lay_hp = QVBoxLayout(tab_help)
        self.btn_toggle_help = QPushButton("Przełącz Widoczność Przycisku [?]")
        self.btn_toggle_help.clicked.connect(self.toggle_help_btn)
        lay_hp.addWidget(self.btn_toggle_help)

        lay_hp.addWidget(QLabel("Dołącz do naszej społeczności:"))
        btn_discord = QPushButton("Otwórz Discord (https://discord.gg/t7aPNRb69v)")
        btn_discord.clicked.connect(lambda: QDesktopServices.openUrl(QUrl("https://discord.gg/t7aPNRb69v")))
        lay_hp.addWidget(btn_discord)
        lay_hp.addStretch()
        self.tabs.addTab(tab_help, "Pomoc")
    ...
    def change_theme_color(self):
        ...
        if selected in COLOR_MAP:
            ...
            QMessageBox.information(self, "Sukces", f"Zmieniono kolor przewodni na {selected}!")

    def change_timezone(self):
        tz = self.combo_tz.currentText()
        self.main_view.set_timezone(tz)
        QMessageBox.information(self, "Strefa czasowa", f"Ustawiono strefę czasową: {tz}")
    ...
    def refresh_users(self):
        ...
        try:
            ...
            self.user_list.addItem("Błąd odczytu /etc/passwd")

    def add_user(self):
        username, ok = QInputDialog.getText(self, "Dodaj Użytkownika", "Nazwa nowego użytkownika:")
        if ok and username:
...
class HelpOverlayItem(QGraphicsObject):
    ...
    def paint(self, painter, option, widget):
        ...
        painter.drawText(QRectF(-230, -190, 460, 40), Qt.AlignmentFlag.AlignCenter, "INSTRUKCJA OBSŁUGI")

        help_text = (
            "• NAWIGACJA I SKRÓTY:\n"
            "  - [SUPER / WINDOWS]: Otwiera/zamyka Menu Aplikacji.\n"
            "  - [PPM na pulpicie]: Menu z opcjami (Ustawienia, Ikony, Tapeta).\n"
            "  - [strzałka WSTECZ]: Wraca do poprzedniego (wyższego) katalogu.\n"
            "  - [strzałka POWRÓT]: Wraca do ikon startowych przypiętych przez użytkownika.\n\n"

            "• MENU PULPITU & USTAWIENIA:\n"
            "  - Możesz dostosowywać ikony tarczy (dodawać/usuwać).\n"  
            "  - Zmieniaj kolor przewodni, strefę czasową i użytkowników.\n"
            "  - Możesz ukryć ten przycisk (help [?]) w ustawieniach w zakładce pomoc.\n\n"

            "• SERWER DISCORD:\n"
            "  - Dołącz do społeczności pod adresem: https://discord.gg/t7aPNRb69v"
        )
        ...
        painter.drawText(QRectF(-60, 150, 120, 35), Qt.AlignmentFlag.AlignCenter, "ZAMKNIJ")
...
class DesktopContextMenuOverlay(QGraphicsObject):
    ...
    def paint(self, painter, option, widget):
        ...
        painter.setPen(THEME_COLOR)
        painter.drawText(QRectF(-100, -125, 200, 25), Qt.AlignmentFlag.AlignCenter, "MENU PULPITU")

        options = ["Terminal", "Dostosuj ikony", "Tapeta", "Ustawienia"]
...
class ApplicationMenuOverlay(QGraphicsObject):
                ...
                if dev.startswith("BAT"):
                    ...
                    if os.path.exists(stat_file):
                        with open(stat_file, "r") as f:
                            status = f.read().strip()
                    status_str = " (Ładowanie)" if status == "Charging" else ""
                    return f"{capacity}%{status_str}", int(capacity) if capacity.isdigit() else 100
        return "Brak baterii / Zasilacz AC", 100
    ...
    def paint(self, painter, option, widget):
        ...
        painter.setFont(QFont("Consolas", 10, QFont.Weight.Bold))
        painter.setPen(THEME_COLOR)
        painter.drawText(QRectF(-230, -175, 210, 25), Qt.AlignmentFlag.AlignCenter, f"WSZYSTKIE ({len(self.filtered_apps)})")
        painter.drawText(QRectF(20, -175, 210, 25), Qt.AlignmentFlag.AlignCenter, "★ ULUBIONE ★")
        painter.drawText(QRectF(20, 10, 210, 25), Qt.AlignmentFlag.AlignCenter, "🕒 OSTATNIO UŻYWANE")
    ...
    def hoverMoveEvent(self, event):
        ...
        if QRectF(-70, 190, 40, 30).contains(pos):
            ...
            QToolTip.showText(QCursor.pos(), f"Bateria: {bat_str}")
        else:
            QToolTip.hideText()
...
class RadialDesktopView(QGraphicsView):
    ...
    def get_default_apps(self):
        ...
        return [
            ...
            RadialIconItem("Przeglądarka", browser_cmd, is_dir=False, icon_name="web-browser"),
            RadialIconItem("Dokumenty", os.path.join(home, "Dokumenty"), is_dir=True, icon_name="folder-documents"),
            RadialIconItem("Pobrane", os.path.join(home, "Pobrane"), is_dir=True, icon_name="folder-download"),
            RadialIconItem("Muzyka", os.path.join(home, "Muzyka"), is_dir=True, icon_name="folder-music"),
            RadialIconItem("Obrazy", os.path.join(home, "Obrazy"), is_dir=True, icon_name="folder-pictures"),
            RadialIconItem("Wideo", os.path.join(home, "Wideo"), is_dir=True, icon_name="folder-videos"),
            RadialIconItem("Kod / Editor", find_cmd(["code", "vscodium", "gedit", "kate"]), is_dir=False, icon_name="text-editor"),
            RadialIconItem("Ustawienia", find_cmd(["gnome-control-center", "systemsettings", "xfce4-settings-manager"]), is_dir=False, icon_name="preferences-system"),
            RadialIconItem("Menedżer", file_mgr_cmd, is_dir=False, icon_name="system-file-manager"),
            RadialIconItem("Dom / Home", home, is_dir=True, icon_name="user-home"),
            RadialIconItem("Kalkulator", calc_cmd, is_dir=False, icon_name="accessories-calculator")
        ]
    ...
    def open_directory(self, path):
        ...
        try:
            ...
            if not new_items:
                new_items.append(RadialIconItem("[Pusty]", "", is_dir=False, icon_name="dialog-information"))
    ...
    def go_home(self):
        """Powraca do głównej tarczy, zachowując aktualną zcustomizowaną listę ikonek."""
        self.history_stack.clear()
        if self.root_items:
            self.render_orbit_items(self.root_items)
        else:
    ...
    def launch_item(self, cmd_or_path, name=None):
        ...
        try:
            ...
            print(f"[BŁĄD] Nie udało się uruchomić '{cmd_or_path}': {e}")
    ...
    def save_config(self):
        ...
        try:
            ...
            print(f"[CUI-DATA] Pomyślnie zapisano konfigurację w {CONFIG_FILE}")
        except Exception as e:
            print(f"[BŁĄD] Nie udało się zapisać konfiguracji: {e}")
    ...
    def load_config(self):
        ...
        try:
            ...
            print(f"[CUI-DATA] Załadowano konfigurację z {CONFIG_FILE}")
        except Exception as e:
            print(f"[BŁĄD] Odczyt konfiguracji nie powiódł się: {e}")
